...
const Missile = forwardRef<MissileHandle, MissileProps>(({ visible = true }, ref) => {
  ...
  useFrame((state) => {
    // Subtle hover only when not in flight (flight animation overrides position)
  });
  ...
  return (
    <group ref={groupRef} scale={0.25}>
      ...
      <mesh position={[0, -2.15, 0]} castShadow>
        ...
        <meshPhysicalMaterial color="#222222" metalness={0.97} roughness={0.08} clearcoat={0.3} />
      </mesh>
      ...
      {[0, Math.PI / 2, Math.PI, (3 * Math.PI) / 2].map((angle, i) => (
        <group key={`fin-${i}`} rotation={[0, angle, 0]}>
          <mesh position={[0.35, -1.6, 0]} rotation={[0, 0, -0.2]} castShadow>
            ...
            <meshPhysicalMaterial color="#4a5060" metalness={0.92} roughness={0.1} clearcoat={0.5} />
          </mesh>
        </group>
      ))}
      ...
      {[0, Math.PI / 2, Math.PI, (3 * Math.PI) / 2].map((angle, i) => (
        <group key={`canard-${i}`} rotation={[0, angle, 0]}>
          <mesh position={[0.25, 1.5, 0]} rotation={[0, 0, -0.15]}>
            ...
            <meshPhysicalMaterial color="#5a6070" metalness={0.9} roughness={0.12} clearcoat={0.4} />
          </mesh>
        </group>
      ))}
    </group>
